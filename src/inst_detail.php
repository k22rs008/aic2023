...
if ($row) {  
    ...
    echo '<tr><th width="20%">機器ID</th><td>' . $row['code'] . '</td></tr>' . PHP_EOL;
    echo '<tr><th>機器名称</th><td>' . $row['fname']. '</td></tr>' . PHP_EOL;
    echo '<tr><th>略称</th><td>' . $row['fshortname']. '</td></tr>' . PHP_EOL;
    $i  = $row['status']; 
    $status= Facility::status; 
    echo '<tr><th>ユーザ種別</th><td>' . $status[$i]. '</td></tr>' . PHP_EOL;
    $i  = $row['category']; 
    ...
    echo '<tr><th>詳細</th><td>' . $row['detail'] . '</td></tr>' . PHP_EOL;
    echo '</table>' . PHP_EOL;
